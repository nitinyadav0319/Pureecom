    ...
    <div class="container">
        <div class="row justify-content-center">
            ...
            <div class="col-lg-5 col-12 tt-login-img"
                data-background="{{ staticAsset('frontend/default/assets/img/banner/banner2.jpeg') }}"></div>

            <div class="col-lg-5 col-12 bg-white d-flex p-0 tt-login-col shadow">
                <form class="tt-login-form-wrap p-3 p-md-6 p-lg-6 py-7 w-100"
                    action="{{ route('vendor.register.submit') }}" method="POST" id="vendor-register-form">
                    @csrf

                    {!! RecaptchaV3::field('recaptcha_token') !!}

                    <h2 class="mb-4 h3">{{ localize('Hey there!') }}<br>{{ localize('Register as a Vendor.') }}</h2>
                    ...
                    {{-- Optional: if you're using roles --}}
                    <input type="hidden" name="user_type" value="vendor">

                    <div class="row g-4 mt-3">
                        <div class="col-sm-12">
                            <button type="submit" class="btn btn-primary w-100 sign-in-btn"
                                onclick="handleVendorSubmit()">{{ localize('Register as Seller') }}</button>
                        </div>
                    </div>

                    <p class="mb-0 fs-xs mt-4">
                        {{ localize('Already have a Vendor Account?') }}
                        <a href="{{ route('login') }}">{{ localize('Login') }}</a>
                    </p>
                </form>
            </div>
        </div>
