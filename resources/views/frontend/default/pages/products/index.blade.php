                ...
                <div class="row g-4">
                    ...
                    <div class="col-xl-9">
                        <div class="shop-grid">
                            ...
                            <!-- Promotional banner -->
@php
    $bannerImage = getSetting('product_page_banner');
    $bannerLink  = getSetting('product_page_banner_link');
@endphp
...
@if ($bannerImage)
    <section class="promotion-banner my-5">
        <div class="container text-center">
            <a href="{{ $bannerLink ?? '#' }}">
                <img src="{{ uploadedAsset($bannerImage) }}"
...
@php
    $widgets = getSetting('product_page_widgets') ? json_decode(getSetting('product_page_widgets')) : [];
@endphp

@if(!empty($widgets))
    <section class="product-widgets py-5">
        <div class="container">
            <div class="row g-4">
                    ...
                    <div class="col-md-4">
                        <div class="card h-100 shadow-sm border-0">
                            @if(!empty($widget->image))
                                <img src="{{ uploadedAsset($widget->image) }}" 
                                     class="card-img-top" 
                                     alt="{{ $widget->title }}">
                            @endif
                            <div class="card-body text-center">
                                <h5 class="card-title">{{ $widget->title }}</h5>
                                <p class="card-text text-muted">{{ $widget->sub_title }}</p>
                            </div>
                        </div>
                    </div>
                @endforeach
