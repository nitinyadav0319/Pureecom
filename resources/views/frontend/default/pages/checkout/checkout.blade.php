...
$(document).ready(function() {
    ...
    // Jab user koi address select kare
    $('input[name="shipping_address_id"]').on('change', function() {
        let addressId = $(this).val();
        if (!addressId) return;

        // Place order button ko temporarily disable karo
        $('#btn_cod').prop('disabled', true);

        $.ajax({
            url: "{{ route('checkout.checkDelivery') }}",
            type: "POST", // POST method use karni hai
            data: {
                _token: "{{ csrf_token() }}",
                address_id: addressId // backend me yehi field check hoti hai
            },
            success: function(response) {
                if (response.available) {
                    // ✅ Delivery available — button enable karo
                    $('#btn_cod').prop('disabled', false);
                } else {
                    // ❌ Delivery unavailable — disable & show message
                    $('#btn_cod').prop('disabled', true);
            ...
            error: function() {
                $('#btn_cod').prop('disabled', true);
                alert('Something went wrong while checking delivery availability.');
            }
        });
    });
