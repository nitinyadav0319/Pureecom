...
<script>
    ...
    // mobile quick actions move to the top while scrolling
    function updateMobileToolbarPosition() {
        if (window.innerWidth <= 767) {
            var startTop = window.innerWidth <= 374 ? 58 : 60;
            var endTop = window.innerWidth <= 374 ? 6 : 8;
            var progress = Math.min(window.scrollY / 70, 1);
            var currentTop = startTop - ((startTop - endTop) * progress);
            var isScrolled = progress >= 0.96;

            $('.mobile-toolbar')
                .css('--mobile-toolbar-top', currentTop + 'px')
                .toggleClass('is-scrolled', isScrolled);
            $('body').toggleClass('mobile-header-scrolled', window.scrollY > 28);
        } else {
            $('.mobile-toolbar').removeClass('is-scrolled').css('--mobile-toolbar-top', '');
            $('body').removeClass('mobile-header-scrolled');
        }
    }
    updateMobileToolbarPosition();
    $(window).on('scroll resize', updateMobileToolbarPosition);
