...
<header class="gheader  z-2">
    ...
    <div class="ghead-topbar d-none d-lg-block" style="background: #062B72">
         <div class="container">  
            <div class="row align-items-center">
                <div class="col-xxl-4 col-xl-3">
                    <div class="topbar-info d-none d-xl-block">
                        <p class="text-white fs-sm fw-medium mb-0 dfdfh" >{{ getSetting('topbar_welcome_text') }}</p>
                    </div>
                </div>
            ...
            <div class="row align-items-center">
                ...
                <div class="col-xxl-10 col-xl-9 col-md-9 col-7">
                    <div class="row d-flex align-items-center">
                        <div class="col-md-8">
                             <div class="gshop-navbar-right align-items-center position-relative">
                        
<div class="gshop-header3">
                      <form class="search-form d-flex align-items-center"
                        ...
                        <div
                            class="gshop-header-icons d-none d-md-inline-flex align-items-center d-flex justify-content-end ms-3">
   ...
   <div class="navnew header-sticky">
            ...
            <nav class="gshop-navmenu ms-3 d-none d-xl-block">
                <ul class="d-flex align-items-center justify-content-center">
                    ...
                    @php
                        ...
                        $allCategories = \App\Models\Category::where('parent_id', 0)
                            ->with('childrenCategories')
                            ->get();

                        // 🔹 Navbar categories (configured in settings)
                        $navbarCategories = [];
                        if (getSetting('navbar_categories') != null) {
                            $navbarCategories = \App\Models\Category::whereIn('id', json_decode(getSetting('navbar_categories')))
                                ->where('parent_id', 0)
                                ->with('childrenCategories')
                                ->get();
                        }

                        // 🔹 Mega Menu Columns from admin
                        $megaMenuColumns = \App\Models\MegaMenuColumn::where('is_active', 1)
                            ->orderBy('order')
                            ->get();
                    @endphp

      <li class="mega-dropdown">
    <a href="javascript:void(0)">{{ localize('All Categories') }}</a>

    <div class="categories-sidebar mega-menu all-categories-mega">
        {{-- Left Column: All Categories --}}
        <div class="mega-left">
            <h6>{{ localize('Shop by Categories') }}</h6>
            <ul class="category-list">
                @foreach($allCategories as $cat)
                    <li class="category-item">
                        ...
                        {{-- Right Columns: Only show on hover --}}
                        @if($cat->megaMenuColumns->count() > 0 || $cat->childrenCategories->count() > 0)
                            <div class="mega-right">
                                {{-- Subcategories --}}
                                @if($cat->childrenCategories->count() > 0)
                                    <div class="mega-column">
                                        <h6>{{ localize('Subcategories') }}</h6>
                                        <ul>
                                ...
                                {{-- Admin added columns --}}
                                @foreach($cat->megaMenuColumns as $column)
                                    <div class="mega-column">
                                        <h6>{{ $column->title }}</h6>

                                        @if($column->type == 'variation' && $column->variation_id)
                                            @php
                                                $variationValues = \App\Models\VariationValue::where('variation_id', $column->variation_id)
                                                    ->where('is_active', 1)
                                                    ->pluck('name');
                                            @endphp
                                            <ul>
                                                @foreach($variationValues as $value)
                                                    <li><a href="{{ route('products.index', ['variation' => $value]) }}">{{ $value }}</a></li>
                                                @endforeach
                                            </ul>
                                                                           @elseif($column->type == 'brand')
    @php
        $brands = \App\Models\Brand::whereIn('id', function($q) use ($cat) {
            $q->select('brand_id')
    ...
    <ul>
        @foreach($brands as $brand)
            <li>
                <a href="{{ route('products.index', ['category_id' => $cat->id, 'brand_id' => $brand->id]) }}">
                    {{ $brand->collectLocalization('name') ?? $brand->name }}
                </a>
            </li>
        @endforeach
    </ul>
        ...
        {{-- Right Side Placeholder (initially blank) --}}
        <div class="mega-right default-right">
            {{-- Empty initially, will show only when category hovered --}}
        </div>
    </div>
</li>
...
@foreach($navbarCategories as $navbarCat)
    <li class="dropdown mega-dropdown">
       <a href="{{ route('products.index', ['category_id' => $navbarCat->id]) }}" class="toggle-mega">
    {{ $navbarCat->collectLocalization('name') }}
</a>

        <div class="categories-sidebar mega-menu nav-category-mega">

            {{-- 🟢 Left Column: Subcategories --}}
            <div class="mega-column">
                <h6>{{ $navbarCat->collectLocalization('name') }}</h6>
                <ul>
                    @foreach($navbarCat->childrenCategories as $subCat)
                        <li>
                            <a href="{{ route('products.index', ['category_id' => $subCat->id]) }}">
                                {{ $subCat->collectLocalization('name') }}
                            </a>
            ...
            {{-- 🟢 Right Columns: Only those assigned to this category --}}
            @php
                $assignedColumns = \App\Models\MegaMenuColumn::where('is_active', 1)
                    ->whereHas('categories', function($q) use ($navbarCat) {
                        $q->where('category_id', $navbarCat->id);
                    })
                    ->orderBy('order')
                    ->get();
            @endphp

            @foreach($assignedColumns as $column)
                <div class="mega-column">
                    <h6>{{ $column->title }}</h6>
                    ...
                    @if($column->type == 'variation' && $column->variation_id)
                        ...
                        <ul>
                            @foreach($variationValues as $value)
                                <li>
                                    <a href="{{ route('products.index', ['category_id' => $navbarCat->id, 'variation' => $value]) }}">
                                        {{ $value }}
                                    </a>
                                </li>
                            @endforeach
                    ...
                    @elseif($column->type == 'brand')
                        @php
                            $brands = \App\Models\Brand::whereIn('id', function($q) use ($navbarCat) {
                                $q->select('brand_id')
                                  ->from('category_brands')
                                  ->where('category_id', $navbarCat->id)
                                  ->distinct();
                            })->get();
                        @endphp
                        <ul>
                            @foreach($brands as $brand)
                                <li>
                                    <a href="{{ route('products.index', ['category_id' => $navbarCat->id, 'brand_id' => $brand->id]) }}">
                                        {{ $brand->collectLocalization('name') ?? $brand->name }}
                                    </a>
                                </li>
                            @endforeach
                    ...
                    @elseif($column->type == 'category')
                        <ul>
                            @foreach($column->categories as $subCat)
                                <li>
                                    <a href="{{ route('products.index', ['category_id' => $subCat->id]) }}">
                                        {{ $subCat->collectLocalization('name') }}
                                    </a>
