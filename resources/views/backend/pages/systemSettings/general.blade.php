...
<div class="card mb-4" id="section-6">
    <div class="card-body">
        <h5 class="mb-4">{{ localize('Login Page Settings') }}</h5>

        <div class="mb-3">
            <label for="login_title" class="form-label">{{ localize('Login Page Title') }}</label>
            <input type="hidden" name="types[]" value="login_title">
            <input type="text" id="login_title" name="login_title" class="form-control"
                   value="{{ getSetting('login_title') }}">
        </div>

        <div class="mb-3">
            <label class="form-label">{{ localize('Login Page Banner') }}</label>
            <input type="hidden" name="types[]" value="login_banner">
            <div class="tt-image-drop rounded">
                <span class="fw-semibold">{{ localize('Choose Login Banner') }}</span>
                <div class="tt-product-thumb show-selected-files mt-3">
                    <div class="avatar avatar-xl cursor-pointer choose-media"
                        data-bs-toggle="offcanvas" data-bs-target="#offcanvasBottom"
                        onclick="showMediaManager(this)" data-selection="single">
                        <input type="hidden" name="login_banner" value="{{ getSetting('login_banner') }}">
                        <div class="no-avatar rounded-circle">
                            <span><i data-feather="plus"></i></span>
                        </div>
                    </div>
