...
//     $vendorShop = auth()->user()->vendorProfile; // ya $vendor_id
//     $orders = $orders->where('vendor_id', auth()->id());
// }
if(auth()->check() && auth()->user()->hasRole('vendor')) {
    $vendorProfileId = auth()->user()->vendorProfile->id; // ✅ vendor_profiles.id
    $orders = $orders->where('vendor_id', $vendorProfileId);
}
