...
class ProductsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($user->user_type === 'vendor') {
            $vendorProfile = \App\Models\VendorProfile::where('user_id', $user->id)->first();
            if (!$vendorProfile) {
                flash(localize('Vendor profile not found, please complete your profile'))->error();
            ...
            $shop = \App\Models\Shop::firstOrCreate(
                ['user_id' => $user->id],
                [
                    'shop_name' => $vendorProfile->business_name ?? $user->name,
                    'slug' => \Illuminate\Support\Str::slug($vendorProfile->business_name ?? $user->name),
                    'is_approved' => 1,
                    'is_published' => 1,
                    'current_balance' => 0,
                ]
            ...
            $vendor_id = null;
            $shop_id = 1; // Admin ki fixed shop id
        ...
        $product->vendor_id = $vendorProfile->id ?? null;
        ...
        // 🔹 Stock Qty
        $product->stock_qty = ($request->has('is_variant') && $request->has('variations'))
    ...
    {      $user = auth()->user();
        ...
        $shop = \App\Models\Shop::firstOrCreate(
            ['user_id' => auth()->id()],
            [
                'shop_name' => auth()->user()->business_name ?? auth()->user()->name,
                'slug' => \Illuminate\Support\Str::slug(auth()->user()->business_name ?? auth()->user()->name),
                'is_approved' => 1,
                'is_published' => 1,
                'current_balance' => 0,
            ]
        ...
        if (!$shop || !$shop->id) {
            throw new \Exception('Shop could not be created for this vendor');
        }
        ...
        if (auth()->user()->user_type === 'vendor' && $product->shop_id != $shop->id) {
        ...
        if ($request->lang_key == env("DEFAULT_LANGUAGE")) {
            ...
            if (auth()->user()->role === 'vendor') {
                $product->status = 'pending';     // vendor ka product hamesha pending
                $product->is_published = 0;       // frontend pe na dikhaye
            } else {
                $product->status = 'approved';    // admin add kare to direct approved
                $product->is_published = $request->is_published ?? 0;
            }
