...
class CartsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // 🔹 Step 1: Check if user has checked valid pincode
        $userPincode = session('user_pincode');
        if (!$userPincode) {
            return response()->json([
                'success' => false,
                'alert' => 'warning',
                'message' => 'Please check your delivery pincode before adding to cart.'
            ]);
        }

        if (!is_null($productVariation)) {

            // 🔹 Step 2: Check if product vendor delivers to this pincode
            $product = $productVariation->product;

            $canDeliver = $product->isDeliverableToPincode($userPincode);

            if (!$canDeliver) {
                return response()->json([
                    'success' => false,
                    'alert' => 'warning',
                    'message' => 'Sorry! This product is not deliverable to your area.'
                ]);
            }

            // 🔹 Step 3: Proceed with normal add-to-cart logic
            $cart = null;
            $message = '';
            ...
            if (is_null($cart)) {
                ...
                $cart->vendor_id = $product->vendor_id; // 👈 Add this
