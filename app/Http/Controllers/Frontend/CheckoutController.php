...
class CheckoutController extends Controller
{
    ...
    public function checkDelivery(Request $request)
    {
        ...
        if (!$address) {
            return response()->json(['available' => false, 'message' => 'Address not found']);
        }
        ...
        if ($carts->isEmpty()) {
            return response()->json(['available' => false, 'message' => 'Cart is empty']);
        }
        ...
        if ($notDeliverable) {
            return response()->json(['available' => false, 'message' => 'Delivery not available for this address']);
        }

        return response()->json(['available' => true, 'message' => 'Delivery available at this address']);
    }
    ...
    public function complete(Request $request)
    {
        ...
        if ($request->payment_method == 'ccavenue') {
        ...
        if (count($carts) > 0) {
            ...
            $order->vendor_id = $product->vendor_id;   // add this
            $order->product_id = $product->id;          // add this
            ...
            foreach ($carts as $cart) {
                ...
                if ($product->vendor_id && $product->vendor_id != auth()->id()) {
                    // Vendor product
                    $category = $product->categories->first(); // ya primary category
                    $commissionRate = $category->commission_percentage ?? 0;

                    $orderItem->admin_commission = ($orderItem->total_price * $commissionRate) / 100;
                    $orderItem->vendor_earning = $orderItem->total_price - $orderItem->admin_commission;
                } else {
                    // Admin product
                    $orderItem->admin_commission = $orderItem->total_price;
                    $orderItem->vendor_earning = 0;
                }
